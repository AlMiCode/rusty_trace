use std::{sync::mpsc::Receiver, mem::take};
...
impl View for RenderedImageView {
    fn title(&self) -> &str {
        match self {
            ...
            Self::Ready { title, colour: _, albedo: _, normal: _, denoised: _, viewed_option: _} => &title
        }
    }
        ...
        match self {
            Self::Waiting { title, rx } => {
                if let Ok(mut img) = rx.try_recv() {
                    let colour = image_to_retained(&DynamicImage::ImageRgb32F(img.colour).into_rgb8());
                    let albedo = image_to_retained(&DynamicImage::ImageRgb32F(img.albedo).into_rgb8());
                    for n in img.normal.as_mut().iter_mut() {
                        *n = (*n + 1.0) / 2.0;
                    }
                    let normal = image_to_retained(&DynamicImage::ImageRgb32F(img.normal).into_rgb8());
                    let denoised = image_to_retained(&DynamicImage::ImageRgb32F(img.denoised).into_rgb8());
                    *self = RenderedImageView::Ready { title: take(title), colour, albedo, normal, denoised, viewed_option: 0u8 }
                }
                ui.spinner();
            },
            Self::Ready { title: _, colour, albedo, normal, denoised, viewed_option } => {
                match viewed_option {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    if ui.selectable_label(*viewed_option == 1u8, "Albedo").clicked() {
                        *viewed_option = 1u8;
                    }
                    if ui.selectable_label(*viewed_option == 2u8, "Normal").clicked() {
                        *viewed_option = 2u8;
                    }
                    if ui.selectable_label(*viewed_option == 3u8, "Denoised").clicked() {
                        *viewed_option = 3u8;
                    }
                });
                ui.horizontal_wrapped(|ui| {
